...
class Driver:
    ...
    def wait_to_switch_iframe(self,locator,retryTime:int=12) -> WebElement:
        return self.wait_to_be(EC.frame_to_be_available_and_switch_to_it,locator,retryTime=retryTime)
    ...
    def switch_to_iframe(self,locator,retryTime:int=10):
        ...
        for i in range(0,retryTime,1):
            try:
                element=WebDriverWait(self.driver,60).until(EC.frame_to_be_available_and_switch_to_it(locator))
                return element
            except TimeoutException:
                self.log.debug("args:"+str(argsDic))
                self.log.error("第 "+str(i+1)+" 次等待超时，刷新页面 ..")
